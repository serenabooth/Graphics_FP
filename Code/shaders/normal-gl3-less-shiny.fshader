...
void main() {
  ...
  vec3 color_tex = texture(uTexNormal, vTexCoord).xyz * 2.0 - vec3(1.0,1.0,1.0); 
  vec3 normal = normalize(vNTMat * color_tex); 
  ...
  vec3 reflection = vec3(0,0,0);//normalize( 1.0 * normal * nDotL - lightDir);
  ...
  //vec3 color = color_tex * diffuse + specular * vec3(0.6, 0.6, 0.6);
